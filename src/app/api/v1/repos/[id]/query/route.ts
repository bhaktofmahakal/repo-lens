...
function buildEvidenceBackedFallbackAnswer(
  ...
  const lines = selectedSnippets
    .map((snippet) => {
      ...
      const targetUrl = snippet.sourceUrl || (repoId ? `/source?sourceId=${encodeURIComponent(repoId)}&path=${encodeURIComponent(snippet.filePath)}${lineHash}` : null);
...
export async function POST(
  ...
  try {
    ...
    const normalizedQuestion = typeof body.question === "string" ? body.question.trim() : "";
    ...
    await checkQueryLimit(auth.userId);
    ...
    if (!hasEvidence) {
      ...
      if (
        ...
        answer = buildEvidenceBackedFallbackAnswer(normalizedQuestion, retrievedSnippets, sourceId);
      }
    ...
    console.error("v1 query API error:", error);
    return NextResponse.json(
